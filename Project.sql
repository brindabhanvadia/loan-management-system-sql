...
create table Customer_Income_Criteria as select * , 
case
when Applicant_Income >15000 then 'Grade_A'
when Applicant_Income > 9000 then 'Grade_B'
when Applicant_Income >5000 then 'Middle_class_Customer'
else 'Low_class' 
end as Customer_Criteria,
case
when Applicant_Income <5000  and  property_Area = 'Rural' then 3.0
when Applicant_Income <5000  and  property_Area = 'SemiRural' then 3.5
when Applicant_Income <5000  and  property_Area = 'Urban' then 5.0
when Applicant_Income <5000  and  property_Area = 'Semiurban' then 2.5
else 7.0
end as Monthly_interest_rate from Customer_income_status;

select * from Customer_Income_Criteria;

create table Loan_Status (Loan_ID varchar(20), Customer_ID varchar(20), Loan_Amount varchar(100), Loan_amount_Term int, CIBIL_Score int);
create table Remark_Update (Loan_ID varchar(20), Loan_Amount varchar(100),CIBIL_Score int , CIBIL_Score_Status varchar(100));

------- Create row level trigger for loan amount -------
delimiter %%
create trigger Loan before insert on Loan_Status for each row
begin
...
delimiter %%
create trigger Remark after insert on loan_status for each row 
begin
if new.cibil_score >900 then insert into Remark_update (loan_id,loan_amount,cibil_score, cibil_score_status)
values (new.loan_id, new.loan_amount, new.cibil_score, "high_cibil_score");

elseif new.cibil_score >750 then insert into Remark_update (loan_id,loan_amount,cibil_score, cibil_score_status)
values (new.loan_id, new.loan_amount, new.cibil_score, "No_penalty");

elseif new.cibil_score >250 then insert into Remark_update (loan_id,loan_amount,cibil_score, cibil_score_status)
values (new.loan_id, new.loan_amount, new.cibil_score, "Penalty_customers");

elseif new.cibil_score <=0 then insert into Remark_update (loan_id,loan_amount,cibil_score, cibil_score_status)
values (new.loan_id, new.loan_amount, new.cibil_score, "Loan_cannot_apply");
end if;
end %%
delimiter ;

select * from remark_update;
...
delete from Loan_Status where Loan_Amount='Loan Still Processing';
delete from Remark_Update where CIBIL_Score_Status='Loan Cannot Apply';

------- Update loan as integers -------
alter table Loan_Status modify column Loan_Amount int;

------- Create all the above fields as a table -------
create table loan_cibil_score_status_details select ls.Loan_ID, ls.Customer_ID, ls.Loan_Amount, ls.Loan_Amount_Term, ls.CIBIL_Score, 
ru.CIBIL_Score_Status from Loan_Status ls left join Remark_Update ru on ls.Loan_ID= ru.Loan_ID;
...
create table Customer_interest_analysis select cic.Loan_ID, cic.Customer_ID, cic.Applicant_Income,cic.Coapplicant_Income,cic.Property_Area,
cic.Loan_Status, cic.Customer_Criteria, ls.Loan_Amount, ls.Loan_amount_term,ls.Cibil_Score, cic.Monthly_interest_rate,
case
when cic.Applicant_Income>15000 then 'Grade A'
when cic.Applicant_Income>9000 then 'Grade B'
when cic.Applicant_Income>5000 then 'Middle Class'
else 'Low Class'
end as Income_Grade,

(cic.Applicant_Income *
case
when cic.Applicant_Income < 5000 and  cic.Property_Area ='Rural' then 3.0
when cic.Applicant_Income < 5000 and  cic.Property_Area ='Semi-Rural' then 3.5
when cic.Applicant_Income < 5000 and  cic.Property_Area ='Urban' then 5.0
when cic.Applicant_Income < 5000 and  cic.Property_Area ='semi-Urban' then 2.5
else 7.0
end /100) as Monthly_Interest_Amount,

((cic.Applicant_Income *
case
when cic.Applicant_Income < 50000 and  cic.Property_Area ='Rural' then 3.0
when cic.Applicant_Income < 50000 and  cic.Property_Area ='Semi-Rural' then 3.5
when cic.Applicant_Income < 50000 and  cic.Property_Area ='Urban' then 5.0
when cic.Applicant_Income < 50000 and  cic.Property_Area ='semi-Urban' then 2.5
else 7.0
end / 100)*12) as Annual_Interest_Amount
from  Customer_Income_Criteria cic join Loan_Status ls on cic.Customer_ID = ls.Customer_ID;

select * from Customer_interest_analysis;

------- Import table Customer info -------
select * from Customer_info;

------- Update gender, age based on a customer id  -------
update customer_info set Gender='Female' where Customer_ID = 'IP43013';
update customer_info set Age=32 where Customer_ID = 'IP43009';

------- Import table Country State and Region -------
select * from country_state;
select * from region_info;

------- Join all the 5 tables without repeating the fields -------
select cis.Loan_ID, cis.Customer_ID, cis.Applicant_Income,cis.Coapplicant_Income,cis.Property_Area,cis.Loan_Status, 
ls.Loan_Amount, ls.Loan_amount_term,ls.Cibil_Score,
ci.Gender,ci.Age, ci.Education, ci.Self_Employed,
cs.Postal_code, cs.Segment, cs.State, cs.Region_Id,
cia.Income_Grade,cia.Monthly_Interest_Amount, cia.Annual_Interest_Amount
from  customer_income_status cis
join Loan_Status ls on cis.Customer_ID = ls.Customer_ID
join Customer_Info ci on cis.Customer_ID = ci.Customer_ID
join  Country_State cs on cis.Customer_ID = cs.Customer_ID
join  customer_interest_analysis cia on cis.Customer_ID = cia.Customer_ID;

------- find the mismatch details using joins -------

select cis.Customer_ID, cis.Loan_ID from Customer_income_Status cis left join Loan_Status ls on cis.Customer_ID = ls.Customer_ID 
where ls.Customer_ID is null;
select cis.Customer_ID, cis.Loan_ID from Customer_income_Status cis left join Customer_Info ci on cis.Customer_ID = ci.Customer_ID
where ci.Customer_ID is null;
select cis.Customer_ID, cis.Loan_ID from Customer_income_Status cis left join Country_State cs on cis.Customer_ID = cs.Customer_ID
where cs.Customer_ID is null;
select cis.Customer_ID, cis.Loan_ID from Customer_income_Status cis left join Customer_interest_analysis cia on cis.Customer_ID = cia.Customer_ID
where cia.Customer_ID is null;



select ls.Customer_ID, ls.Loan_ID from Loan_Status ls left join Customer_income_Status cis on ls.Customer_ID = cis.Customer_ID
where cis.Customer_ID is null;
select ls.Customer_ID, ls.Loan_ID from Loan_Status ls left join Customer_Info ci on ls.Customer_ID = ci.Customer_ID
where ci.Customer_ID is null;
select ls.Customer_ID, ls.Loan_ID from Loan_Status ls left join Country_State cs on ls.Customer_ID = cs.Customer_ID
where cs.Customer_ID is null;
select ls.Customer_ID, ls.Loan_ID from Loan_Status ls left join Customer_interest_analysis cia on ls.Customer_ID = cia.Customer_ID
where cia.Customer_ID is null;



select ci.Customer_ID, ci.Customer_Name from Customer_Info ci left join Customer_income_Status cis on ci.Customer_ID = cis.Customer_ID
where cis.Customer_ID is null;
select ci.Customer_ID, ci.Customer_Name from Customer_Info ci left join Loan_Status ls on ci.Customer_ID = ls.Customer_ID
where ls.Customer_ID is null;
select ci.Customer_ID, ci.Customer_Name from Customer_Info ci left join Country_State cs on ci.Customer_ID = cs.Customer_ID
where cs.Customer_ID is null;
select ci.Customer_ID, ci.Customer_Name from Customer_Info ci left join Customer_interest_analysis cia on ci.Customer_ID = cia.Customer_ID
where cia.Customer_ID is null;



select cs.Customer_ID, cs.State from Country_State cs left join customer_income_status cis on cs.Customer_ID = cis.Customer_ID
where cis.Customer_ID is null;
select cs.Customer_ID, cs.State from Country_State cs left join Loan_Status ls on cs.Customer_ID = ls.Customer_ID
where ls.Customer_ID is null;
select cs.Customer_ID, cs.State from Country_State cs left join Customer_Info ci on cs.Customer_ID = ci.Customer_ID
where ci.Customer_ID is null;
select cs.Customer_ID, cs.State from Country_State cs left join customer_interest_analysis cia on cs.Customer_ID = cia.Customer_ID
where cia.Customer_ID is null;



select cia.Customer_ID, cia.Applicant_Income from customer_interest_analysis cia left join customer_income_status cis on cia.Customer_ID = cis.Customer_ID
where cis.Customer_ID is null;
select cia.Customer_ID, cia.Applicant_Income from customer_interest_analysis cia left join Loan_Status ls on cia.Customer_ID = ls.Customer_ID
where ls.Customer_ID is null;
select cia.Customer_ID, cia.Applicant_Income from customer_interest_analysis cia left join Customer_Info ci on cia.Customer_ID = ci.Customer_ID
where ci.Customer_ID is null;
select cia.Customer_ID, cia.Applicant_Income from customer_interest_analysis cia left join Country_State cs on cia.Customer_ID = cs.Customer_ID
where cs.Customer_ID IS NULL;


select 'Loan_Status' as Mismatch_Type, cis.Customer_ID,cis.Loan_ID,cis.Applicant_income
from Customer_income_Status cis left join Loan_Status ls on cis.Customer_ID = ls.Customer_ID where ls.Customer_ID is null
union all
select 'Customer_Info' AS Mismatch_Type, cis.Customer_ID, cis.Loan_ID,cis.Applicant_income
from Customer_income_Status cis left join Customer_Info ci on cis.Customer_ID = ci.Customer_ID where ci.Customer_ID is null
union all
SELECT  'Country_state' AS Mismatch_Type, cis.Customer_ID, cis.Loan_ID, cis.Applicant_income
from Customer_income_Status cis left join  Country_State cs on cis.Customer_ID = cs.Customer_ID where  cs.Customer_ID is null
union all
select 'Customer_interest_analysis' as Mismatch_Type, cis.Customer_ID, cis.Loan_ID, cis.Applicant_income
from Customer_income_Status cis left join Customer_interest_analysis cia on cis.Customer_ID = cia.Customer_ID where cia.Customer_ID is null;


------- Filter high cibil score -------
select max(CIBIL_Score) as Highest_CIBIL_Score from Loan_Status;

select cis.Customer_ID, cis.Applicant_Income, 
ls.Loan_ID, ls.Loan_Amount, ls.CIBIL_Score, 
ci.Education, ci.Self_Employed, 
cs.State,cs.Region_id,cs.Postal_Code, 
ri.Region 
from Customer_income_Status cis inner join Loan_Status ls on cis.Customer_ID = ls.Customer_ID
inner join Customer_Info ci on cis.Customer_ID = ci.Customer_ID
inner join Country_State cs on cis.Customer_ID = cs.Customer_ID
inner join Region_Info ri on cs.Region_id = ri.Region_id
where ls.CIBIL_Score = (select max(CIBIL_Score) from Loan_Status);
...
delimiter %%
create procedure MismatchDetails()
begin
select 'Loan_Status' as Mismatch_Type, cis.Customer_ID,cis.Loan_ID,cis.Applicant_income
from Customer_income_Status cis left join Loan_Status ls on cis.Customer_ID = ls.Customer_ID where ls.Customer_ID is null
union all
...
delimiter ;
call MismatchDetails();

delimiter %%
create procedure HighestCibilScore()
begin
select cis.Customer_ID, cis.Applicant_Income, 
ls.Loan_ID, ls.Loan_Amount, ls.CIBIL_Score, 
ci.Education, ci.Self_Employed, 
...
delimiter ;
call  HighestCibilScore();


delimiter %%
create procedure OfficeType()
begin
select cis.Customer_ID, cis.Applicant_Income, 
ls.Loan_ID, ls.Loan_Amount, ls.CIBIL_Score, 
ci.Education, ci.Self_Employed, 
